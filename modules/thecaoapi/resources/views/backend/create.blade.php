...
@section('breadcrumb')
    ...
    <a class="breadcrumb-item active" href="{{ route('thecaoapi.create') }}">@lang('global.add')</a>
@endsection
...
@section('content')
    <div class="br-section-wrapper">
        ...
        {{-- Bg content --}}
        <form action="{{ route('thecaoapi.store') }}" method="post" enctype="multipart/form-data" id="frm_create_thecaoapi">
            @csrf
...
@section('script')
    <script>
        $('#frm_create_thecaoapi').on('submit', function (event) {
            event.preventDefault();

            var form = $('#frm_create_thecaoapi');
            ...
            createThecaoapi(form.serialize());
        });

        $('#frm_create_thecaoapi').validate({
            errorElement: "span",
            rules: {
                name: {
                    required: true
        ...
        function createThecaoapi(data) {
            $.ajax({
                url: app_url + 'admin/thecaoapi',
