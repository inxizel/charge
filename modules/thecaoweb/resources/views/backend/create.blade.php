...
@section('breadcrumb')
    ...
    <a class="breadcrumb-item active" href="{{ route('thecaoweb.create') }}">@lang('global.add')</a>
@endsection
...
@section('content')
    <div class="br-section-wrapper">
        ...
        {{-- Bg content --}}
        <form action="{{ route('thecaoweb.store') }}" method="post" enctype="multipart/form-data" id="frm_create_thecaoweb">
            @csrf
...
@section('script')
    <script>
        $('#frm_create_thecaoweb').on('submit', function (event) {
            event.preventDefault();

            var form = $('#frm_create_thecaoweb');
            ...
            createThecaoweb(form.serialize());
        });

        $('#frm_create_thecaoweb').validate({
            errorElement: "span",
            rules: {
                name: {
                    required: true
        ...
        function createThecaoweb(data) {
            $.ajax({
                url: app_url + 'admin/thecaoweb',
