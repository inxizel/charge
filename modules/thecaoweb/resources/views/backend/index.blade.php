...
@section('breadcrumb')
    <a class="breadcrumb-item active" href="{{ route('thecaoweb.index') }}">{{ $display_name }}</a>
...
@section('content')
    <div class="br-section-wrapper">
        ...
        <div class="col-sm-2 col-md-2 pd-0">
            <button class="btn btn-info btn-block mg-b-20" onclick="window.location='{{ route('thecaoweb.create') }}'">
        ...
        <div class="rounded table-responsive">
            <table class="table table-bordered mg-b-0" id="thecaoweb_table">
                <thead>
                <tr>
...
@section('script')
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#thecaoweb_table').DataTable({
                autoWidth: true,
                processing: true,
                serverSide: true,
                ordering: false,
                ajax: {
                    url: app_url + 'admin/thecaoweb/get_list_thecaoweb',
                    type: 'post'
                },
                searching: true,
                columns: [
            ...
            $('#thecaoweb_table').on('click', '.btn-edit', function () {
                window.location.href = app_url + 'admin/thecaoweb/' + $(this).data('id') + '/edit';
            });

            $('#thecaoweb_table').on('click', '.btn-delete', function (event) {
                ...
                swal({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: app_url + 'admin/thecaoweb/' + $(this).data('id'),
                            type: 'DELETE',
                            dataType: "JSON",
                            data: {
                                id: $(this).data('id')
                            ...
                            success: function (res)
                            {
                                if (!res.err) {
                                    ...
                                    $('#thecaoweb_table').DataTable().ajax.reload();
                                } else {
